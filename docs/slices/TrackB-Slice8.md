# Track B — Slice 8: [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) SKILL.md authoring
...
plus the on-demand files under `reference/`, and closes the [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) criteria that are checkable
...
does the skill fire, does it produce the right operators — is [Slice 9](./TrackB-Slice9.md)'s, measured in fresh
sessions. This slice writes the hypothesis; it does not get to claim the result.

## Preconditions

The dependency graph ([`docs/DEV-ROADMAP.md`](../DEV-ROADMAP.md) [§5](../DEV-ROADMAP.md#5-order-and-parallelism)) puts one edge into this slice: [Slice 3](./TrackA-Slice3.md), which
...
- `src/tools/register.ts` — the bare tool name `card_search`, its compact description, and its
  JSON Schema ([Slice 5](./TrackA-Slice5.md)). The description is already in the model's context every session; the
  skill's job is to extend it, not restate it.
- [`docs/slices/TrackA-Slice6-results.md`](./TrackA-Slice6-results.md) — live evidence, 13/13 checks, for the operators, the
...
**Not preconditions.** An installed plugin ([Slice 7](./TrackB-Slice7.md)), an eval harness ([Slice 9](./TrackB-Slice9.md)), and a
`claude plugin details` measurement ([Slice 10](./TrackC-Slice10.md)) are all absent and all fine to be absent. Slice 8
...
[PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) is the plugin-side complement to the server's [CAP-01](../MCP-PRD.md#cap-01--card-search): CAP-01 makes the search possible,
[PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) makes it good. Without it, the tools require the user to know the thing the plugin exists
...
| [`docs/DEV-ROADMAP.md`](../DEV-ROADMAP.md) | modify — Slice 8 row to ☑, its three done-when boxes, and a Landed note. Nothing else |
...
**No PRD edit.** This slice resolves no open question. [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft)'s §7/§9 record and MCP-PRD's [OQ-01](../MCP-PRD.md#oq-01--how-should-scryfall-syntax-be-surfaced-to-the-model)
update both belong to [Slice 9](./TrackB-Slice9.md), when the numbers exist. Recording static measurements in the
...
1. **Frontmatter: three keys, and none you cannot validate.** `name: scryfall-query-craft`
   (matching the directory — the harness prefixes it, which is what makes [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft)'s stated
   `/manabase:scryfall-query-craft` surface work), `description`, and `when_to_use`. Do **not**
   set `disable-model-invocation`: [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) exists to fire unprompted, and that is the entire
   ...
   scoped form from **Interface contracts** (PLUGIN-PRD [P-12](../PLUGIN-PRD.md#p-12--plugin-name-and-server-key)). Write any boolean as
   `true`/`false`; the `yes`/`no`/`on`/`off` aliases need harness 2.1.218, above the plugin's
   2.1.207 floor (PLUGIN-PRD [§3.2](../PLUGIN-PRD.md#32-minimum-harness-version)). Invent no other keys — `claude plugin validate . --strict`
   is the check, and it is a standing gate anyway ([`docs/DEV-ROADMAP.md`](../DEV-ROADMAP.md) [§3](../DEV-ROADMAP.md#3-standing-rules--apply-to-every-slice-never-restated-per-slice), rule 7).
...
2. **The `description` + `when_to_use` wording must match plain Magic questions that never say
   ...
   *query*, or *syntax*. Put the key use case first (PLUGIN-PRD [§3.1](../PLUGIN-PRD.md#31-context-budget): the listing truncates at
   ...
   **This wording is a hypothesis, not a result.** [Slice 9](./TrackB-Slice9.md) measures should-trigger against
   should-not-trigger rate ([PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) criteria 5 and 11) and tunes it. Write it as one rewritable
   block of prose rather than something structurally load-bearing, so tuning it is cheap.

3. **Gate: `description` + `when_to_use` ≤1,536 characters** ([PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) criterion 1; PLUGIN-PRD
   [§3.1](../PLUGIN-PRD.md#31-context-budget)'s `skillListingMaxDescChars`). **Characters, not tokens.** Measure with verification
   ...
   **A planning figure, not a gate:** [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) criterion 2 caps always-on cost at ≤250 tokens, and
   that is measured in **[Slice 10](./TrackC-Slice10.md)** by `claude plugin details manabase`, not here. At roughly
   ...
   combined characters and record the count so [Slice 10](./TrackC-Slice10.md) can correlate. 1,536 is a cap on what
   the harness will display, never a target.

4. **Gate: `SKILL.md` body ≤5,000 tokens. Target ≤2,000.** The ceiling is [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) criterion 3 and
   PLUGIN-PRD [§3.1](../PLUGIN-PRD.md#31-context-budget): after compaction the harness re-attaches only the **first 5,000 tokens** of
   ...
   **A body past the ceiling silently loses its tail.** There is no error and no warning — the
   ...
   ≤2,000 figure is [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft)'s on-invoke target and the number to design against; treat any body
...
5. **How to count tokens — pick an instrument and record which one.** In order of preference:
   ...
   instrument `claude plugin details` uses (PLUGIN-PRD [§4.6](../PLUGIN-PRD.md#46-context-cost-accounting)) — recording the model id alongside
...
6. **The split rule, stated once and applied everywhere.** A line belongs in `SKILL.md` only if
   ...
   harness-verified mechanism (PLUGIN-PRD [§4.1](../PLUGIN-PRD.md#41-harness-features-relied-on)) and it only works if the body names each
   ...
   is verified (PLUGIN-PRD [§4.1](../PLUGIN-PRD.md#41-harness-features-relied-on)), resolution of a relative path from an arbitrary cwd is not.
   Reference files must live **inside** the skill directory — an installed plugin cannot
   reference `../` paths at all (PLUGIN-PRD [§3.3](../PLUGIN-PRD.md#33-trust-and-sandboxing)).
...
8. **`reference/recipes.md` — worked translations and the failure loop.** English request → the
   query it becomes, one line each, covering the request shapes [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) criteria 6–9 probe:
...
9. **Body: the English-request-to-query procedure.** Not a list of operators — a procedure the
   ...
   one query rather than being split into a query plus client-side filtering — [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) criterion 6
...
10. **Body: the high-frequency operators only.** The ones that appear in most queries live in the
    ...
    traceable.** The verified set is MCP-PRD [§4.1.1](../MCP-PRD.md#411-search-endpoint)'s table plus the queries exercised live in
    [`docs/slices/TrackA-Slice6-results.md`](./TrackA-Slice6-results.md): `t:`, `o:`, the regex form `o:/…/`, `f:`, `cmc` with
...
11. **Body: the failure loop.** The server returns Scryfall's own `details` text verbatim on a
    malformed query (MCP-PRD [D-10](../MCP-PRD.md#d-10--tool-handlers-never-throw), [CAP-01](../MCP-PRD.md#cap-01--card-search)). The skill directs Claude to **read it, revise the
...
12. **Body: the operators that plausibly do not exist.** `illustrationtag:` is the named case —
    ...
    (MCP-PRD [§4.1.1](../MCP-PRD.md#411-search-endpoint), verified twice). [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) criterion 10 is a negative check that it never
...
13. **Body: the parameters that change meaning, not formatting.** `unique`, `order`, and `dir`
    ([CAP-01](../MCP-PRD.md#cap-01--card-search)). `unique=cards` is the default and is a deckbuilding default — one row per card, not
...
15. **Every tool reference uses the scoped form** `mcp__plugin_manabase_mtg__card_search`
    (PLUGIN-PRD [P-12](../PLUGIN-PRD.md#p-12--plugin-name-and-server-key)). The server registers the bare `card_search`; the harness scopes it, and
...
16. **Skills carry instructions, never card facts** (PLUGIN-PRD [§3.6](../PLUGIN-PRD.md#36-skills-carry-instructions-never-facts) — the sharpest rule in this
    ...
    - **Operator counts are the most tempting and the most wrong.** MCP-PRD [§4.1.1](../MCP-PRD.md#411-search-endpoint) records real
...
17. **This slice makes [OQ-01](../MCP-PRD.md#oq-01--how-should-scryfall-syntax-be-surfaced-to-the-model)'s bet explicit — and must not assume its answer.** MCP-PRD OQ-01
    ...
    split works.** [Slice 9](./TrackB-Slice9.md) measures it. Two consequences here: do not restate the tool
    ...
    [Slice 9](./TrackB-Slice9.md)'s finding to act on, not this slice's to pre-empt.
...
18. **Change nothing under `src/`, and do not rebuild `dist/`.** The skill teaches the tool as it
    ...
    the results doc and raise against MCP-PRD — not a change to make here (PLUGIN-PRD [§1](../PLUGIN-PRD.md#1-overview)'s
...
19. **Record the evidence** in `docs/slices/TrackB-Slice8-results.md`: date, Node and Claude Code
    ...
    MCP-PRD [§4.1.1](../MCP-PRD.md#411-search-endpoint)'s verified table, the card-fact review record (requirement 20), and an
    explicit statement that criterion 2 is [Slice 10](./TrackC-Slice10.md)'s and criteria 5–13 are [Slice 9](./TrackB-Slice9.md)'s, neither
    claimed here.

20. **The card-fact review has a reviewer and a checklist** — otherwise [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) criterion 4 is
    ...
    actually claims, which is the same reason [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) requires fresh sessions for its behavioral
...
The scoped form is the only form the skill uses ([P-12](../PLUGIN-PRD.md#p-12--plugin-name-and-server-key)).

**Parameters** (`q` required, everything else optional):

| Parameter | Type | Meaning |
|---|---|---|
| `q` | string, **required** | The Scryfall query. Sent verbatim — never parsed, validated, or rewritten by the server (MCP-PRD [D-07](../MCP-PRD.md#d-07--three-way-cache-split)). Scryfall evaluates the syntax, which is why the full language works and why a bad query comes back as Scryfall's own error text |
...
Repo layout is unchanged from the [Slice 1](./TrackA-Slice1.md) doc; this slice adds only files under
`skills/scryfall-query-craft/` and one results doc.

## Out of scope — do NOT

- **Do not touch `src/`, `dist/`, `tests/`, `.mcp.json`, `plugin.json`, `marketplace.json`, or
  `README.md`.** No rebuild, no version bump, no tool-description edit (that is [Slice 9](./TrackB-Slice9.md)'s call to
  make if the measurement demands it).
- **Do not write evals, run evals, or claim a behavioral criterion.** [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) criteria 5–13 need
  fresh sessions and a baseline comparison; a self-assessment from the authoring session is the
  specific thing [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft)'s method preamble rules out. [Slice 9](./TrackB-Slice9.md) owns them.
- **Do not run `claude plugin details` and record a cost figure.** That is [Slice 10](./TrackC-Slice10.md), and it needs
  the installed plugin.
- **Do not edit [`docs/PLUGIN-PRD.md`](../PLUGIN-PRD.md) or [`docs/MCP-PRD.md`](../MCP-PRD.md).** No open question resolves here. The
  only doc edits are the results file and the roadmap's status bookkeeping.
- **No second skill, no command, no agent, no hook.** [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) is one skill (PLUGIN-PRD [P-07](../PLUGIN-PRD.md#p-07--skills-not-commands), [§8](../PLUGIN-PRD.md#8-out-of-scope)).
...
- **Do not provoke a 429 while verifying operators.** ≥600 ms between card-endpoint calls, one
  ...
  application being banned for every user (MCP-PRD [§3.4](../MCP-PRD.md#34-rate-limits-are-hard-constraints-not-guidance)). This is binding, not advisory.

## Acceptance criteria

1. **[[PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) #1]** `description` + `when_to_use` measures ≤1,536 characters. The number and the
   counting rule are in the results doc.
2. **[[PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) #3]** The `SKILL.md` body measures ≤5,000 tokens by the recorded instrument. The
...
3. **[[PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) #4]** A fresh reviewer with no authoring context runs requirement 20's seven-point
...
5. Every operator named as real in any skill file is either in MCP-PRD [§4.1.1](../MCP-PRD.md#411-search-endpoint)'s verified table
...
- the operator verification log — every operator not already in MCP-PRD [§4.1.1](../MCP-PRD.md#411-search-endpoint)'s table, checked
...
be produced in this session at all: [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft)'s method requires fresh sessions against a recorded
without-skill baseline, and authoring context masks exactly the gaps the eval is looking for.
[Slice 9](./TrackB-Slice9.md) does that work. **Do not write a "seems to work" note in place of it.**

## Verification steps

Git Bash. Steps 1–4 are the measurements; step 5 is per-operator and only for operators outside
MCP-PRD [§4.1.1](../MCP-PRD.md#411-search-endpoint)'s verified table. These are one-off commands and none of them is committed — if
...
- [`docs/DEV-ROADMAP.md`](../DEV-ROADMAP.md) [§4](../DEV-ROADMAP.md#4-phase-1-slices), Slice 8 (goal and done-when source); [§5](../DEV-ROADMAP.md#5-order-and-parallelism) (why this slice needs only
  [Slice 3](./TrackA-Slice3.md)); [§3](../DEV-ROADMAP.md#3-standing-rules--apply-to-every-slice-never-restated-per-slice) (standing rules — never restated per slice).
- [`docs/PLUGIN-PRD.md`](../PLUGIN-PRD.md) [PC-01](../PLUGIN-PRD.md#pc-01--scryfall-query-craft) (behavior and criteria 1–4; 5–13 are [Slice 9](./TrackB-Slice9.md)'s), [§3.1](../PLUGIN-PRD.md#31-context-budget) (context
  budget — the 1,536-character cap and the 5,000-token compaction window), [§3.6](../PLUGIN-PRD.md#36-skills-carry-instructions-never-facts) (skills carry
  instructions, never facts), [§4.1](../PLUGIN-PRD.md#41-harness-features-relied-on) (skills, supporting files, `${CLAUDE_SKILL_DIR}`, scoped tool
  names, `/reload-plugins` for a marketplace-installed plugin), [§3.2](../PLUGIN-PRD.md#32-minimum-harness-version) (harness version floor),
  [§4.6](../PLUGIN-PRD.md#46-context-cost-accounting) (how `plugin details` produces its numbers), [P-07](../PLUGIN-PRD.md#p-07--skills-not-commands), [P-12](../PLUGIN-PRD.md#p-12--plugin-name-and-server-key).
- [`docs/MCP-PRD.md`](../MCP-PRD.md) [OQ-01](../MCP-PRD.md#oq-01--how-should-scryfall-syntax-be-surfaced-to-the-model) (the syntax-surfacing bet this slice makes and [Slice 9](./TrackB-Slice9.md) measures),
  [CAP-01](../MCP-PRD.md#cap-01--card-search) (field list, pagination, meaning-changing parameters, structured failures), [§4.1.1](../MCP-PRD.md#411-search-endpoint)
  (verified operators, the invalid `illustrationtag:`, and counts that drift), [§4.1.3](../MCP-PRD.md#413-price-fields--three-verified-traps) (the price
  traps behind `PriceInfo`), [§3.4](../MCP-PRD.md#34-rate-limits-are-hard-constraints-not-guidance) (rate limits are binding while verifying operators), [D-07](../MCP-PRD.md#d-07--three-way-cache-split),
  [D-10](../MCP-PRD.md#d-10--tool-handlers-never-throw).
- [`docs/slices/TrackA-Slice6-results.md`](./TrackA-Slice6-results.md) — the live evidence behind every operator and price
  behavior the skill is allowed to assert.
- [`docs/slices/TrackA-Slice3.md`](./TrackA-Slice3.md) and [`TrackA-Slice5.md`](./TrackA-Slice5.md) — canonical `CardSearchData` /
