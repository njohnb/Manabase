# Track A — Slice 6: Live [CAP-01](../MCP-PRD.md#cap-01--card-search) acceptance pass
...
**Goal.** Exercise all 12 [CAP-01](../MCP-PRD.md#cap-01--card-search) acceptance criteria against **live Scryfall**, at polite
...
recorded in [`docs/MCP-PRD.md`](../MCP-PRD.md) [§9](../MCP-PRD.md#9-revision-log).

## Preconditions (deliverables of [Slice 5](./TrackA-Slice5.md))
...
| [`docs/slices/TrackA-Slice6-results.md`](./TrackA-Slice6-results.md) | new — the recorded run |
| [`docs/MCP-PRD.md`](../MCP-PRD.md) | append one revision-log row to [§9](../MCP-PRD.md#9-revision-log) (template below) |
...
3. **The check matrix.** Expected values are the 2026-07-29 baselines from [`MCP-PRD.md`](../MCP-PRD.md)
   [§4.1.1](../MCP-PRD.md#411-search-endpoint); thresholds are deliberately loose so ordinary drift passes while breakage fails.

   | # | Query / action | Pass condition | [CAP-01](../MCP-PRD.md#cap-01--card-search) criterion | 2026-07-29 baseline |
   |---|---|---|---|---|
   ...
   | 8 | `illustrationtag:dragon` | `isError: true`, parsed body `error.code === "bad_request"`, `error.details` contains "All of your terms were ignored" — and the *next* check still succeeds (server survived) | 8 (structured failure, [D-10](../MCP-PRD.md#d-10--tool-handlers-never-throw)) | HTTP 400 |
   ...
   | 13 | `name:"xyzzynocardhasthisname"` | success with `cards: []`, `total_cards: 0`, `note` present — validates the Slice 3 decision that Scryfall's zero-match 404 maps to an empty success | (supplementary — not a [CAP-01](../MCP-PRD.md#cap-01--card-search) criterion) | Scryfall returns HTTP 404 for zero matches |
...
6. **Record the run** in [`docs/slices/TrackA-Slice6-results.md`](./TrackA-Slice6-results.md): date, Node version, the
...
7. **Close the loop in the PRD.** Append one row to the [`docs/MCP-PRD.md`](../MCP-PRD.md) [§9](../MCP-PRD.md#9-revision-log) revision-log
...
8. **If a check fails**, fix the code in the slice that owns it (client → [Slice 2](./TrackA-Slice2.md) spec,
   handler → [Slice 3](./TrackA-Slice3.md), prices → [Slice 4](./TrackA-Slice4.md), wiring → [Slice 5](./TrackA-Slice5.md)), rebuild `dist/`, and re-run the
   ...
   to reality, and say so in the [§9](../MCP-PRD.md#9-revision-log) row.
...
`isError: true` on failure. Canonical shapes are in the [Slice 3](./TrackA-Slice3.md) and [Slice 5](./TrackA-Slice5.md) docs. Repo layout
is unchanged from the [Slice 1](./TrackA-Slice1.md) doc; this slice adds only `scripts/cap01-live.mjs`.
...
- No edits to [`docs/MCP-PRD.md`](../MCP-PRD.md) beyond the single appended [§9](../MCP-PRD.md#9-revision-log) row; no plugin-file changes.
- No new dependencies.
...
3. [`docs/slices/TrackA-Slice6-results.md`](./TrackA-Slice6-results.md) exists with the recorded table, observed values,
   evidence split, and Drift section.
4. [`docs/MCP-PRD.md`](../MCP-PRD.md) [§9](../MCP-PRD.md#9-revision-log) has the new row; `git diff docs/MCP-PRD.md` shows exactly one
...
- [`docs/DEV-ROADMAP.md`](../DEV-ROADMAP.md) [§4](../DEV-ROADMAP.md#4-phase-1-slices), [Slice 6](../DEV-ROADMAP.md#slice-6--live-cap-01-acceptance-pass).
- [`docs/MCP-PRD.md`](../MCP-PRD.md) [§5](../MCP-PRD.md#5-capabilities) [CAP-01](../MCP-PRD.md#cap-01--card-search) (the 12 criteria — the contract this slice closes), [§4.1.1](../MCP-PRD.md#411-search-endpoint)
  (operator baselines), [§4.1.3](../MCP-PRD.md#413-price-fields--three-verified-traps) (price traps), [§3.4](../MCP-PRD.md#34-rate-limits-are-hard-constraints-not-guidance) (politeness is binding), [§9](../MCP-PRD.md#9-revision-log) (revision log).
