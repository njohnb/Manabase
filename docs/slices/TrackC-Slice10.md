...
details` baseline that `PC-02` criterion 10 requires, a verdict on `PC-01` criterion 2 (the
≤250-token always-on cost), and definite answers to `PQ-01` (do an MCP server's tool schemas
count toward the reported always-on total?) and `PQ-02` (does this plugin fit alongside
...
**Slice 7 — install verification.** Binding. Every instrument in this slice reads an *installed*
...
listing for `/doctor` to price. Slice 7 also fixes the marketplace and plugin names this slice
must type on the command line.

**Slice 8 — `PC-01` `SKILL.md` authoring.** Not drawn in the roadmap's §5 graph (which draws only
7 → 10), and the graph is right that this slice is *runnable* without it: a plugin with zero
skills and one MCP server still reports an always-on total, so the `PQ-01` A/B and the `PC-02`
...
- It **is** worth the `PQ-01` answer. That experiment varies `.mcp.json` and nothing else; the
  presence or absence of a skill is not a variable in it.
- It **is** worth a `PC-02`-only baseline — the cost of the plugin shell (`plugin.json`
  description, keywords) plus whatever the server contributes.
- It is **not** worth `PC-01` criterion 2. Before `SKILL.md` exists there is no always-on skill
  cost to compare against ≤250 tokens; the criterion is vacuous, not passed.
- It is **not** worth `PQ-02`'s answer. `PQ-02` asks whether *this plugin's* contribution fits in
...
Two legal orders, therefore. Either run the whole slice after Slice 8, or run it in two passes:
**pass 1** before the skill (`PQ-01` + the `PC-02` shell baseline), **pass 2** after it (`PC-01`
criterion 2 + `PQ-02`). If two passes, each records its own conditions block and pass 1's totals
...
the server (`CAP-01` delivered, live, 2026-08-03); Track B proves the install and writes the
skill; Track C measures what the pair costs every session, keeps the committed `dist/` honest
(Slice 11), and ships (Slices 12–13). Slice 10 feeds Slice 12, whose README work documents
`/doctor` as the answer to `PQ-04` — the "how would a friend notice their listing was trimmed?"
...
Three facts from `PLUGIN-PRD.md` that make this slice necessary rather than bookkeeping, inlined
because they shape every number below:

- **The skill listing is capped and the failure is silent** (§3.1). `skillListingBudgetFraction`
...
- **`claude plugin details <name>` is the only instrument** (§4.6). It reports a component
  ...
  fires).
- **The instrument's numbers are uneven in quality** (§4.6). The always-on total is computed via
...
The grounding measurement §4.6 already records, for scale: `claude plugin details
...
per skill. That is the number `PQ-02` asks Manabase to fit alongside.
...
| `docs/PLUGIN-PRD.md` §7 | modify — `PQ-01` and `PQ-02` closed with their measured answers (or rewritten to say what was measured and why it did not resolve) |
| `docs/PLUGIN-PRD.md` §5 | modify — `PC-01`'s and `PC-02`'s **Context cost** bullets gain the measured figures beside their estimates. Nothing else in §5 changes |
| `docs/PLUGIN-PRD.md` §9 | append exactly one revision-log row (template in requirement 10) |
| `docs/DEV-ROADMAP.md` §4 Slice 10 + §4 status table | modify — status ☑, both done-when boxes ticked, one **Landed:** note. Sequencing only; the PRD holds the substance |
| `.mcp.json` | **temporarily moved and restored** during the `PQ-01` experiment — net zero change. `git status` must show it unmodified when the slice ends |
...
1. **This slice measures; it does not fix.** No behavior changes. If a measured number fails a
   gate, the fix belongs to the slice that owns the surface — `PC-01` criterion 2 is a property
   of the skill's `description` + `when_to_use`, which is Slice 8's deliverable, and tool
   description length is Slice 5's. A fix there invalidates the measurement, so the loop is
...
2. **Every recorded number carries its conditions.** A bare token count is unreproducible and
   cannot be compared against a later run — which matters, because §4.6 says to re-run at each
   phase boundary rather than treating the figures as durable. Each measurement in the results
   doc gets a conditions block stating: **date**; **`claude --version`** (the §3.2 floor is
   ...
   **every plugin and marketplace enabled at the time**; **the Manabase commit SHA installed**
   (`version` is deliberately unset during development per `P-08`, so the SHA *is* the version);
...
4. **The measurement matrix.**
   ...
   |---|---|---|---|---|
   | 1 | Plugin baseline | `claude plugin details` for the installed Manabase plugin, in a terminal | Full verbatim output; component inventory (counts by type, including the MCP-server count); plugin-level always-on total; per-component always-on and on-invoke; any sign the character-based fallback was used | `PC-02` criterion 10; supplies **A₁** for measurement 4 |
   | 2 | `PC-01` always-on cost | The `PC-01` skill's always-on row in measurement 1's output | The figure, and an explicit verdict against ≤250 tokens: pass / fail / ambiguous-because-scaled | `PC-01` criterion 2 |
   | 3 | Skill listing text size | Count the characters of `description` + `when_to_use` in `SKILL.md`'s frontmatter — **characters, not tokens**; the 1,536 cap is a character cap | The count, and its margin under 1,536 | Corroborates `PC-01` criterion 1 (owned by Slice 8) and explains measurement 2's figure |
   | 4 | `PQ-01` A/B | The reversible controlled experiment of requirement 5 | **A₁** (with `.mcp.json`), **B** (without), **A₂** (restored); the refresh step that made the harness notice; the MCP-server count in each inventory | `PQ-01` |
   ...
   | 6 | Post-budget listing size | `/context`, same session | Verbatim Skills row (listing size *after* the budget is applied) plus the overall breakdown for scale | `PQ-02` |
   | 7 | Manabase-only control *(optional)* | Disable every other plugin, re-run `/doctor` and `/context` | The delta against measurements 5–6 | A **control** for `PQ-02`, explicitly not its answer |
...
5. **The `PQ-01` experiment is a reversible, controlled A/B.** It is the delicate part of this
   ...
   1. **Record the before state.** `git status` clean. Run measurement 1 in full. Note the
      ...
      — §4.6's grounding measurement shows the inventory reports counts by component type
   ...
   **What the answer means, either way — record the implication, not just the number.**
   ...
   - **If A ≠ B** (removing the server lowered the reported total), tool schemas *do* count.
     `PQ-01` says why that matters: unlike a skill description, a tool schema **cannot** be
     budget-trimmed (§3.1's degradation applies to the skill listing, not to tool definitions),
     ...
     Slice 5 warning ("keep tool count and description length lean") from prudence into a
     standing constraint on every future capability, and gives `MCP-PRD.md` `OQ-01` (should the
   ...
   - **If A = B**, `plugin details` does not account for tool schemas. This is the outcome §4.6
     ...
     it means the instrument cannot see them, so `PC-02`'s always-on figure stays
...
6. **`PQ-02` is measured under the author's real load, not a clean profile.** Run `/doctor` and
   `/context` with everything the author normally has enabled. The question is aggregate: §3.1's
   ...
   whether any of them is Manabase's), and — `PQ-02`'s own phrasing — whether §3.1's silent
...
7. **`PC-01` criterion 2 is a gate this slice can fail.** ≤250 tokens always-on. Three things
   follow, and the spec is written assuming failure is a real outcome:

   - **The fix is not here.** `PC-01`'s always-on cost *is* its `description` + `when_to_use`
     text; the only lever is shortening that text, and that text is Slice 8's deliverable. On a
     fail, re-open Slice 8, tune, and re-run measurements 1–3. Each iteration is a new
     ...
     been the first.
   - **Slice 9 tunes the same text for a different objective.** Its eval loop adjusts the
   ...
   - **A figure hovering at the line is ambiguous, not a pass.** Per §4.6, per-component
     always-on numbers are proportionally scaled from the plugin total rather than measured, so
     `PC-01`'s reported figure is indicative. Say so in the results doc, and treat anything within
     a plausible scaling error of 250 as ambiguous — with the plugin-level total, which *is*
     trustworthy, recorded beside it.

8. **Reject a fallback measurement.** §4.6: if the `count_tokens` API is unreachable, the command
   falls back to a character-based estimate. Such a run is not comparable to a `count_tokens` run,
   is not comparable to §4.6's grounding measurement, and must never be mixed into the `PQ-01`
...
9. **Get the argument form right and write it down.** §4.6's grounding measurement used the
   qualified `plugin@marketplace` form (`claude plugin details dotnet-plugin@dotnet-plugin`),
   while `PC-02` criterion 10 and the roadmap both write the bare `claude plugin details
   manabase`. Try the bare name; if it is ambiguous or not found, use the qualified form —
   `P-11` makes the repo its own marketplace, so the marketplace name is also `manabase`. Record
...
10. **Close the loop in `docs/PLUGIN-PRD.md`, in the same session.** Both `PQ-01` and `PQ-02`
    ...
    resolve, and what would. §7's own rule: *questions stay here until answered — they are not
    dropped.* Do not delete either heading and do not renumber. **Do not reword either heading
    either**: the two PRDs are densely cross-linked by GitHub heading anchors, `PQ-01`'s anchor is
    long, and it is referenced from `PC-02`, §4.6, §9, and the roadmap. Renaming it breaks every
    one of those links silently.

    `PC-02`'s **Context cost** bullet currently reads "magnitude currently unverified. Basis: none
    available" — once `PQ-01` is answered it has a basis, so update it. `PC-01`'s bullet gains the
    measured always-on figure beside its ~150–250 estimate. **§2 and §3 are locked** — inherited,
    not re-litigated — and §4 is a dated research record: if reality diverged from §4.6, append a
    dated addendum rather than overwriting it. A measured answer to an open question is a §7
    update plus a §9 row; it is **not** a new `P-` decision and **not** a new `PC` block. Never
    duplicate a decision's text — reference the ID.

    Then append exactly one row to §9 (append-only; change nothing else in that table):
...
11. **Where the full CLI dump goes — resolve this tension explicitly, do not paper over it.**
    `PC-02` criterion 10 says `claude plugin details manabase` "runs and its output is recorded in
    §9." Read literally that puts a multi-line CLI dump inside a three-column markdown table whose
    every other row is a paragraph of prose. §9's rows are one-line summaries with a Why column; a
    pasted terminal dump breaks both the table and the log's shape. The resolution, and it is
    stated in the §9 row itself so no later reader mistakes it for a shortcut:
    ...
    - The **§9 row** carries the figures that matter — totals, the pass/fail verdict, the two `PQ`
      answers — and points at the results document **by path**.
    - This is the precedent Slice 6 set for the `CAP-01` live pass, whose 13-check matrix lives in
      `docs/slices/TrackA-Slice6-results.md` while `MCP-PRD.md` §9 carries one summarizing row.
    - Criterion 10's substance — the measured baseline exists, is recorded verbatim, and is
      ...
      session disagrees and wants the dump inline in §9, that is a PRD edit belonging to whoever
...
- `.claude-plugin/plugin.json` — `name: manabase`, a `displayName`, a `description` carrying the
  verbatim Fan Content disclaimer, an `author`, and five `keywords`. No `version` (`P-08`), no
  `userConfig` (`P-13`).
- `.claude-plugin/marketplace.json` — one plugin entry, `source: "./"` (`P-11`), the same
  disclaimer, a `category`, five `tags`.
- `.mcp.json` — one stdio server, key `mtg`, `node ${CLAUDE_PLUGIN_ROOT}/dist/index.js`. **This is
  the single file the `PQ-01` experiment moves and restores.**
- `src/tools/register.ts` — one tool, `card_search`: a five-line description naming the operator
  ...
  `PQ-01` is asking about.** Whatever the answer, it is the cost of exactly one tool with one
...
- No source changes, no `dist/` rebuild, no schema edit, no skill-text edit, no manifest edit. A
  failed gate is re-opened Slice 8 (skill description) or Slice 5 (tool description) — not an
...
- Do not leave the harness modified. Plugins disabled for the optional control get re-enabled;
  the model is not switched mid-run; the plugin is left installed exactly as Slice 7 left it.
...
- No edits to `docs/MCP-PRD.md`. If `PQ-01` comes back "schemas count," that is a real constraint
  on future tool count and description length — but recording it there is that document's own
  session. Name the implication in `PLUGIN-PRD.md` §9's Why column and stop; do not spec around
  it, and do not reopen `OQ-01` from here.
- No CI wiring and no automated cost-regression check. Slice 11 owns CI, and a recurring cost
  check is not scheduled — §4.6's instruction is to re-run at each phase boundary by hand.
- No attempt to raise `skillListingBudgetFraction` on a user's behalf. §3.1: a plugin's root
...
1. `docs/slices/TrackC-Slice10-results.md` exists and, for every measurement taken, contains the
   ...
   `SKILL.md` existed.
2. **`PC-02` criterion 10 satisfied:** `claude plugin details` ran against the installed plugin,
   its complete output is recorded, and `PLUGIN-PRD.md` §9 points to it by path — with the row
   itself saying that is where the dump lives.
3. **`PC-01` criterion 2 has an explicit verdict** — pass, fail, or ambiguous-because-scaled —
   stated with the measured number and the plugin-level total beside it. Not "looks fine."
4. **`PQ-01` has a definite answer** backed by three measurements — A₁ with `.mcp.json`, B
   ...
   occurred.
5. **`PQ-02` has an answer stated as a judgment** about headroom under the author's full load,
   with the enabled-plugin list recorded, and it says in words whether §3.1's silent degradation
   is live or theoretical on this machine.
6. `docs/PLUGIN-PRD.md`: §7's `PQ-01` and `PQ-02` updated or closed; `PC-01`'s and `PC-02`'s
   context-cost bullets carry measured figures; §9 has exactly one new appended row. `git diff
...
8. `docs/DEV-ROADMAP.md` §4's Slice 10 entry is ☑ with both done-when boxes ticked and a
...
- **The A/B is self-testing.** `A₂ = A₁` is the assertion. A run that fails it is discarded, not
  ...
  evidence about the instrument's stability and §4.6 already warns these numbers are only as
...
- `docs/DEV-ROADMAP.md` §4 Slice 10 (goal and done-when), §5 (the graph: 7 → 10 → 12), §3
  (standing rules, never restated per slice), §2 (current state).
- `docs/PLUGIN-PRD.md` §3.1 (the context budget and its silent failure), §3.2 (version floor;
  `/context` skill accounting correct from 2.1.196), §4.6 (what `plugin details` reports, the
  proportional scaling of per-component figures, the character-based fallback, and the
  `dotnet-plugin` grounding measurement), `PC-01` criterion 2, `PC-02` criterion 10, `PQ-01`,
  `PQ-02`, §7 (questions are never dropped), §9 (append-only revision log).
- `docs/slices/TrackA-Slice6.md` and `docs/slices/TrackA-Slice6-results.md` — the precedent for a
...
- `docs/MCP-PRD.md` `OQ-01` (how Scryfall syntax should reach the model) — `PQ-01`'s answer gives
