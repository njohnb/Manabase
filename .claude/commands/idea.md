...
## The entry shape — 13 fields, fixed order
...
| 7 | **Depends on** | research | `CAP`/`PC`/slice/decision IDs, and any `OQ`/`PQ` whose answer this idea waits on |
| 8 | **Constrained by** | **research** | The locked `D-`/`P-` decisions and §3 standing rules it must respect |
| 9 | **Conflicts / prior art** | **research** | Mandatory §8 check on both PRDs, plus §6 packs and existing `IDEA-` entries |
| 10 | **Opens** | interview | Questions the idea raises but cannot answer. **Pre-staged only** — they get `OQ`/`PQ` numbers when the idea is triaged into a block, not before |
| 11 | **Value** | interview | What it unlocks, in the author's voice |
| 12 | **Size, rough** | interview | `one slice` · `a pack` · `a one-liner` · `unknown`. Deliberately coarse |
| 13 | **Next step** | research + interview | The smallest concrete action that moves it, and whether it is desk work |

**Fields 8 and 9 are researched, never asked of the author.** They carry most of this command's
...
**Field 10 is where the two lanes touch, and it touches in one direction only.** An idea may
...
`docs/DEV-ROADMAP.md` §6. That fills fields 7, 8 and 9 and seeds 6 and 13. **Report what you found
...
`AskUserQuestion` for the structured forks — problem framing, scope boundary, likely home, size —
...
- **You never delete an entry.** A promoted or dropped entry keeps its row with a pointer to where
  it went.
- **Field 6 is a guess and says so in the text you write.** Applying `PLUGIN-PRD` §1's boundary
  rule: if the idea would need the server to do something it does not do yet, that is a `CAP` in
  `MCP-PRD`, not a `PC` — say so, per `CLAUDE.md`.
